 ...
 *
 * @param items A list of LocationModel objects representing the locations to display.
 */
class LocationAdapter2(private val items: List<LocationModel>) :
    RecyclerView.Adapter<LocationAdapter2.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        context = parent.context // Save the context for later use
        val binding =
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // Set default values for the rating
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (locationSnapshot in snapshot.children) {
                    ...
                    if (name?.trim() == location.name?.trim()) {
                        ...
                        Log.d("LocationAdapter2", "Fetched rating: $rating for ${location.name}")
                        return
                    }
                }
                // Log and handle case where no matching location is found
                Log.d("LocationAdapter2", "No matching location found for ${location.name}")
                holder.binding.ratingTxt.text = "N/A"
                holder.binding.ratingBar.rating = 0f
            }

            override fun onCancelled(error: DatabaseError) {
                // Log and handle errors during database access
                Log.e("LocationAdapter2", "Failed to fetch rating for ${location.name}", error.toException())
                holder.binding.ratingTxt.text = "N/A"
                holder.binding.ratingBar.rating = 0f
            }
        })
        ...
        Glide.with(holder.itemView.context)
            .load(location.picture) // Load the image URL
